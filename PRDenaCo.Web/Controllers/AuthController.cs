using PRDenaCo.Web.Models.Auth;
...
namespace PRDenaCo.Web.Controllers
{
    public class AuthController : Controller
    {
        ...
        public AuthController(IUserLoginService userLoginService, IUserRegisterService userRegisterService,
            IUserActiveAccountService userActiveAccountService, IMessageSender messageSender, IFlushableMemoryCache memoryCache, IUserProfileService userProfileService, IUserForgotService userForgotService, IUserUpdatePassService userUpdatePassService)
        {
                ...
                var claims = new List<Claim>()
            {
              ...
              //  new Claim(ClaimTypes.Email, signupResult.Data.Email),
               ...
               // claims.Add(new Claim(type: "OrgEmp_Id", value: signupResult.Data.OrgEmp_Id.ToString()));
                ...
                var activeUser = new ActiveUser() { Accounts_Id = signupResult.Data.Accounts_Id,
            ...
            if (ModelState.IsValid)
            {
                ...
                if (resultDto.IsSuccess == true)
                {
                    if (resultDto.Message == AppMessages.USER_REGISTER_SUCCESS)
                    {
                        ...
                        //string uname = resultDto.Data.Accounts_Id.ToString() + "_" + resultDto.Data.Users_Id.ToString();
                        //var emailConfirmationToken = Cryptography.Encrypt( uname, "DumiSoft.Com");
                        //var emailMessage = "<html><body> <div style=\"font-size:medium;float:left;direction:ltr;padding-top:50px\">" + "Hi <span style=\"font-weight: bold\" >" + viewModel.Name + "</span> <br/>" + " Your Password :<span style=\"font-weight: bold\" >" + passresult + "</span><br/>" + "Please Verify Your Email Before Login " + "<br/> <a href=\" " +
                        //    Url.Action("ConfirmEmail", "Auth",
                        //        new { token = emailConfirmationToken },
                        //        Request.Scheme) + "\" > Confirm Now </a>"
                        //+ "<div >" + "</body></html>";
                        // _messageSender.SendEmailAsync(viewModel.Email, "Dumi Soft Email Confirmation", emailMessage,true);
